...
class BDistMethod:
    ...
    def __init__(self, knowledge_base, input_letters: List[Letter], bdist: int):
        ...
        self.bdist = bdist
        self.representatives: dict[str, Word] = {}
    ...
    def find_counterexample(self, hypothesis: Automata):
        ...
        self.__compute_representatives(hypothesis)

        for q in hypothesis.get_states():
            for letter in self.input_letters:
                word = Word([letter])
                w_i = self.representatives[q.name] + word
                ...
                w_i_prime = self.representatives[q_prime.name]
                if w_i == w_i_prime:
                    continue

                suffix, query_i, query_i_prime = self.__check_equivalence(w_i, w_i_prime)
                if suffix:
                    expected_output_word = hypothesis.play_word(w_i + suffix)[0].letters
                    if expected_output_word != query_i.output_word.letters:
                        query = query_i
                    else:
                        expected_output_word = hypothesis.play_word(w_i_prime + suffix)[0].letters
                        query = query_i_prime
    ...
    def __compute_representatives(self, automaton: Automata):
        ...
        self.representatives = {currentState.name: Word([])}
        to_analyze = self.__add_candidates_to_analyze(currentState)

        while to_analyze and len(self.representatives) < nb_states:
            (currentState, in_word) = to_analyze.pop()
            if currentState.name not in self.representatives.keys():
                self.representatives[currentState.name] = in_word
                to_analyze = to_analyze + self.__add_candidates_to_analyze(currentState)

    def __add_candidates_to_analyze(self, currentState):
        return [
            (t.output_state, self.representatives[currentState.name] + Word([t.input_letter]))
    ...
    def __check_equivalence(self, w_i: Word, w_i_prime: Word):
        for suffix in self.suffix_list:
            ...
            if (query_i.output_word.last_letter() != query_i_prime.output_word.last_letter()):
